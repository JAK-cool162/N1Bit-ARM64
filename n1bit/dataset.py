...
class DatasetEngine:
    ...
    def search_datasets_on_web(self, query: str, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        discovered = []
        try:
            results = self.fetch_web_json(search_api_url)
            for item in results:
                repo_id = item.get("id")
            ...
            print(f"[Search Engine Error] Failed to scrape dataset repository listings: {e}")
    ...
    def fetch_hf_repo_files_pure_python(self, repo_id: str) -> List[Dict[str, Any]]:
        ...
        Integrated with the Smart Resumable Downloader to show download bars.
        """
        samples = []
        try:
            ...
            for file_path in text_files[:2]:
                ...
                # Run the Smart Resumable Downloader with speed & ETA feedback!
                self.download_web_file(download_url, local_dest, label=file_path)
    ...
    def process_all_datasets(self, force_refresh: bool = False, selected_repos: List[str] = None):
        ...
        self.stats = {
            ...
            "size_after_bytes": os.path.getsize(self.processed_data_file),
